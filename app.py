...
def getpage(page, id):
    sql = "select * from Pages where page_id = %s" % (id)
    cursor.execute(sql)
    pages = cursor.fetchall()
    page['id'] = id
    page['name'] = pages[0][1]
    page['description'] = pages[0][2]


def getpageposts(userdata, id):
    sql = "select content, time_stamp, p_id, photo from Posts where page_id=%s" % (
        id)
    cursor.execute(sql)
    posts = cursor.fetchall()
    posts = posts[::-1]
...
def getusermessages(messagedata, room):
    ...
    sql = "select Content,name,media,timestamp from Messages,Users  where Messages.`From`=Users.id and room_id= %s order by timestamp" % (
        room)
    cursor.execute(sql)
...
def getuserpages(userdata):
    # pages user isn't a part of
    sql = "select * from Pages where page_id not in (select page_id from Following where user_id=%s)" % (
        session['userid'])
    cursor.execute(sql)
    userdata['otherpages'] = cursor.fetchall()


def getallpages(userdata):
    # pages which user is a part of
    sql = "select * from Pages where page_id in (select page_id from Following where user_id=%s)" % (
        session['userid'])
    cursor.execute(sql)
    userdata['userpages'] = cursor.fetchall()
...
def home():
    ...
    getuserpages(userdata)
    getallpages(userdata)
...
def myprofile():
    if request.method == 'GET':
        if 'logged_in' in session:
            ...
            d = session['userid']
            cursor.execute(
                "select * from Posts where u_id=%s and community is null" % str(d))
            data = cursor.fetchall()
...
def handle_send_message_event(data):
    ...
    cursor.execute(sql, (data['user_id'], data['room'],
                         data['friend_id'], data['message']))
    mydb.commit()
...
@app.route('/page/<string:choice>/<string:id>')
def page(choice, id):
    if choice == "join":
        sql = "insert into Following values (%s, %s)" % (id, session['userid'])
        cursor.execute(sql)
        mydb.commit()
        return redirect(request.referrer)

    sql = "delete from Following where page_id = %s and user_id = %s" % (
        id, session['userid'])
    cursor.execute(sql)
    mydb.commit()
    return redirect(request.referrer)
...
def createpage():
    ...
    sql = "insert into Pages (name, description) values ('%s','%s')" % (
        name, desc)
    cursor.execute(sql)
    mydb.commit()
    return redirect(request.referrer)
...
@app.route('/pageposts/<string:id>', methods=['GET', 'POST'])
def pageposts(id):
    if request.method == 'GET':
        if not auth('/pageposts/%s' % (id)):
            redirect('/login')
        sql = 'Select * from Following,Users where page_id='+id+' and user_id=id;'
        cursor.execute(sql)
        users = cursor.fetchall()
        p_users = [item[0] for item in users]
        session['currentpage'] = id
        userdata = {}
        getuserdata(userdata)
        getuserpages(userdata)
        getpageposts(userdata, id)
        page = {}
        getpage(page, id)
        return render_template("Page.html", userdata=userdata, user=session['userid'], page=page, users=users, auth=session)
    else:
        content = request.form['content']
        if not request.files['picture']:
            photo = "None"
        ...
        cursor.execute(
            "insert into Posts (u_id, content, photo, page_id) values (%s,%s,%s,%s)", (session['userid'], content, photo, id))

        mydb.commit()
        return redirect("/pageposts/"+session['currentpage'])
