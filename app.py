...
def getcommunityposts(userdata,id):
	sql = 'select name,content,time_stamp,p_id,photo from Users,Posts where community = %s and u_id = id and (u_id in (select u2_id from Friends where u1_id =%s) or u_id = %s)'%(id,session["userid"],session["userid"])
	cursor.execute(sql)
	posts = cursor.fetchall()
	posts = posts[::-1]	# Newest posts come first
	userdata['posts'] = posts
...
def groupposts(id):
	if request.method =='GET':
		if not auth('/groupposts/%s'%(id)): redirect('/login')
		...
		if session['userid'] not in c_users:
			sql2 = "INSERT INTO Belongs VALUES('%s','%s');"%(session['userid'],id)
			cursor.execute(sql2)
			mydb.commit()
		...
		if not request.files['picture']:
			...
			uploads_dir = os.path.join(app.static_folder, 'postimg/'+str(session['userid']))
			os.makedirs(uploads_dir, exist_ok=True)
			f = request.files['picture']
			f.save(os.path.join(uploads_dir, secure_filename(f.filename)))
